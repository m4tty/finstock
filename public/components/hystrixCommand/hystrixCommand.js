...
(function(window) {
	...
	window.HystrixCommandMonitor = function(containerId, args) {
		...
		function calcRatePerSecond(data) {
			...
			data["ratePerSecond"] = data["requestCount"]; 	//roundNumber(totalRequests / numberSeconds);
			data["ratePerSecondPerHost"] = roundNumber(data["ratePerSecond"] / data["reportingHosts"]) ;
	    }
